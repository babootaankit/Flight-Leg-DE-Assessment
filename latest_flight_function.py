...
def get_latest_flight(filepath: str) -> pd.DataFrame:
    ...
    df = pd.read_excel(filepath, sheet_name='Dummy_Flight_Leg_Data', header=7, usecols=cols, engine='openpyxl')

    # creat datetime with lastupdt and flight dt
    # Use errors='coerce' to drop bad dates
    df['lastupdt'] = pd.to_datetime(df['flight_dt'].astype(str) + ' ' + df['lastupdt'].astype(str), errors='coerce')

    df = df.dropna(subset=['lastupdt', 'flightkey'])

    # sort and drop duplicate values
    df = df.sort_values(['flightkey', 'lastupdt'])
    latest_flight_df = df.drop_duplicates(subset='flightkey', keep='last').reset_index(drop=True)

    # order columns and drop ones not needed
    latest_flight_df = latest_flight_df[['flightkey', 'flightnum', 'flight_dt',
                   'orig_arpt', 'dest_arpt', 'flightstatus', 'lastupdt']]
